...
class AgentService:
    ...
    def create_agent(self, agent: AgentDto):
        ...
        self.graph_dict['agent'].add_node(
            agent.name, props={
                'actions': [
                    a.name for a in agent.actions]})
    ...
    def add_active_tags(self, tags: List[str]) -> List[TagDto]:
        ...
        tags_resp = [
            TagDto(
                **t.dict(),
                group_rank=rankings.get(
                    t.group)) for t in self.agent.state.tags]
        return tags_resp
    ...
    def find_path(self, source_node, target_node, lenses=[]):
        ...
        if not lenses:
            ...
            filtered_graph = self.composer.apply_lenses(
                lenses, flag_infeasible=True)

        unfiltered_graph = self.composer.get_composed_graph()
        return find_path_with_feasibility(
    ...
    def run(self, name) -> NextActionResponse:
        ...
        targets, goals, focus_tags = self.agent.run_graph_and_get_targets(
            self.composer)
        paths = []
            ...
            nodes_resps = map(
                lambda x: [
                    PathNode(
                        node=p['node'],
                        type=p['type'] or None,
                        infeasible=p['infeasible']) for p in x],
                path)
            for nodes_resp in list(nodes_resps):
                path_resp.append(Path(path=nodes_resp, goal=goal, feasible=all(
                    [not n.infeasible for n in nodes_resp])))
