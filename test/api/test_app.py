...
def test_find_succesful_path():
    ...
    active_tags_response = client.post(f"/agent/{name}/active_tags", json=["health_high", "zombie_far", "food_inventory_high", "food_high", "got_shelter", "no_credit"])
...
def test_run_agent_succesfully():
    ...
    with open("./test/api/samplelinks.json", 'r') as f:
      ...
      links_response = client.post(f"/{name}/tag_links", json=data)
      assert links_response.status_code == 200
    active_tags_response = client.post(f"/agent/{name}/active_tags", json=["health_high", "zombie_close", "food_inventory_high", "food_high", "got_shelter", "no_credit", "no_tools"])
    assert active_tags_response.status_code == 200
    with open("./test/api/samplestate.json", 'r') as f:
      data = AgentMCState(**json.load(f))
      state_response = client.post(f"/{name}/update_state", json=data.dict())
...
def test_run_agent_succesfully_multi_runs():
    ...
    active_tags_response = client.post(f"/agent/{name}/active_tags", json=["health_low", "zombie_close", "food_inventory_high", "food_high", "got_shelter", "no_credit", "got_tools"])
    assert active_tags_response.status_code == 200
    response = client.post(f"/agent/{name}/run")
    assert response.status_code == 200
    data = NextActionResponse(**response.json())
    ...
    active_tags_response = client.post(f"/agent/{name}/active_tags", json=["health_high", "zombie_far", "food_inventory_high", "food_high", "got_shelter", "no_credit", "got_tools"])
    assert active_tags_response.status_code == 200
    response = client.post(f"/agent/{name}/run")
    assert response.status_code == 200
    data = NextActionResponse(**response.json())
