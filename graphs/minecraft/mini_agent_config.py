...
def only_trades(x): return True if 'action' in x[0] and 'trade' not in x[0] else False
def only_bid(x): return True if x[0] == 'trade:ask' else False
def only_ask(x): return True if x[0] == 'trade:bid' else False
...
def one_to_many_join_graphs(graph_dict): return {
    'sources': [
        ('actions',
         graph_dict['actions']),
        ('trade',
