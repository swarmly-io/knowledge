    ...
    def inventory_state_runner(self, state: AgentMCState, inventory_graph):
        ...
        for k, i in inv_state.items():
            ...
            inventory_graph.add_node(
                f"{val}", props={**{'quantity': i, 'name': val}, 'joins': joins})

    def observation_state_runner(
            self,
            state: AgentMCState,
            observations_graph):
        ...
        for o in obs_state:
            if o.type == "item":
                ...
                observations_graph.add_node(
                    f"{val}", props={**{'name': val}, 'joins': joins})
            if o.type == 'mob':
                ...
                observations_graph.add_node(
                    f"{o.name}", props={**{'name': o.name, 'type': o.type}, 'joins': joins})
