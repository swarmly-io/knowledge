...
class Agent:
    def __init__(
        ...
        self.goal_valuation: GoalValuation = GoalValuation(
            self.all_tags, self.groups)
    ...
    def run_graph_and_get_targets(self, composer: GraphComposer):
        ...
        for action, index, subindex, node in set(targets):
            resolved_target = self.resolve_target(
                composer, action, index, subindex, node)
    ...
    def resolve_target(
        ...
        for n in graph.nodes():
            if subindex:
                neighbours = [
                    nei for nei in nx.neighbors(
                        graph, n) if subindex in nei]
                if not neighbours:
                    continue
    ...
    def make_graph(self):
        ...
        for action in self.actions:
            actions_graph.add_node(action.name,
                                   **{"props": {"name": action.name}})

        tags_graph = nx.Graph()
        for tag in self.all_tags:
            tags_graph.add_node(
                action.name, **{"props": {"name": tag.name, "type": tag.type}})
    ...
    # need priority service to limit goals - but priority service also needs
    # to make a focus graph
    def focus_graph_and_get_targets(self,
                                    composer: GraphComposer,
                                    goals: List[GoalStatement],
                                    focus_tags: List[Tag]) -> List[Tuple[str,
                                                                         str]]:
        ...
        return composer.link_goals_and_get_targets(
            goals, focus_tags, enriched_links)
