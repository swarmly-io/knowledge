...
class GoalValuation:
    ...
    # returns intersection of selected goal and tags (success != active,
    # failure == active)
    def select(
            self,
    ...
    def score_goals(self,
                    goals: List[GoalStatement],
                    active_tags: List[Tag]) -> List[Tuple[float,
                                                          float,
                                                          GoalStatement]]:
        needs_multiplier_dict = self.calculate_needs_multiplier(active_tags)
        chosen_goals = []
        for goal in goals:
            failure_tags, failure_score, success_tags, success_score = self.filter_tags(
                goal, active_tags, needs_multiplier_dict)
        ...
        return list(map(lambda x: (
            x[0], x[1], x[-1]), sorted(chosen_goals, key=lambda x: (x[0], x[1]), reverse=True)))
    ...
    def calculate_needs_multiplier(self, active_tags: List[Tag]):
        ...
        lowest_score = min(
            [self.group_absolute_rankings.get(a.group) for a in active_tags])
        print([self.group_absolute_rankings.get(a.group) for a in active_tags])
        return {
            g: lowest_score /
            self.group_absolute_rankings[g] for g in self.group_absolute_rankings}

    # todo use Tag type in calculation
    def score(self, tag: Tag, need_multiplier_dict: Dict[str, any]):
        score = self.tag_score_fn.get(tag.group)(
            self.groups_lookup[tag.group], tag)
    ...
    def tag_scoring_functions(
            self, tags: Dict[str, List[Tag]], groups: List[Group]):
