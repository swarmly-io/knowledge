...
class Agent:
    ...
    def setup_graphs(self):
        ...
        for graph in self.graphs.values():
            graph.composer.update_decisions_graphs(
                self.name, self.goals, self.actions, self.all_tags)
    ...
    async def run_state(self, state_update: Any):
        if isinstance(state_update, AgentMCState):
            ...
            G, F = graph.composer.value_graph(
                [], graph.composer.get_composed_graph(), flag_feasibility=True)
    ...
    def run_graph(self) -> Tuple[List[str], List[GoalStatement], List[Tag], Any]:
        ...
        resolved_targets, goals, focus_tags = graph.composer.get_path_targets(
            self.name, feasible_graph, self.goals, self.actions, goals, focus_tags, self.tag_links)

        max_score = max([scores.get(s).get('failure') + scores.get(s).get('success')
                        for s in scores])
    ...
    def target_to_path(self, max_paths: int, targets: List[str], paths: list):
        ...
        # todo a fully connected graph constrained by all goals an agent has, work
        # backwards to create tags and tag links
        ...
        for p in paths:
            ...
            if sub_paths:
                mapped_sub_paths = [
                    SubPath(
                        ...
                        feasibility=sp[3],
                        paths=[
                            make_path(p) for p in sp[4]]) for sp in sub_paths]
            else:
                mapped_sub_paths = []
            ...
            for nodes_resp in nodes_resps:
                ...
                if feasible:
                    ...
                    not_feasible_paths.append(
                        Path(
                            path=nodes_resp,
                            goal=goal,
                            feasible=feasible,
                            sub_paths=mapped_sub_paths))
    ...
    def get_feasibility(self, target: WorkflowTarget) -> Feasibility:
        ...
        return get_feasibility(feasible_graph, fully_connected_graph,
                               self.name, target.node, lenses=[])
    ...
    def get_priority(self, tags: List[str]):
        ...
        def score(t): return self.goal_valuation.score(t, needs_multiplier_dict)
